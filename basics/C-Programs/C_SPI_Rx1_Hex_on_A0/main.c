...
int main(void)
{
   WDTCTL = WDTPW | WDTHOLD;    //stop watchdog timer
    ...
    P2DIR &= ~BIT3;                 // make p4.1 an input (SW1)
  ...
  //SW 2 (PORT6)
  P6IE |= BIT6;                      // set local P6 enable
  P6IFG &= ~BIT6;                    // clear interrupt flag for IRQ
  ...
  UCAIFG &= ~UCRXIFG;               // clear flag for IRQ
...
{ 
  ...
  //send dummy bit to generate clock 
  UCA0TXBUF = dummybyte;            // Master send dummy byte
  dummybyte_sent = 1; 
  ...
  if (dummybyte_sent == 1 && dummybyte_received == 1)
   {
    ...
    if (UCA0IFG & UCTXIFG)     //TX buffer is empty 
    {  
